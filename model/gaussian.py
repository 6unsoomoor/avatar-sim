# gaussian_refac.py — CPU-only version. 파일로 변경됨. renderer_refac.py 경로 문제로 일단 복붙해놓음.
...
def linear_blending_torch(weight, xyz, rot, feat, xyz_b, rot_b, feat_b):
    """
    CUDA kernel (linear_blending.cu) and mathematically equivalent CPU implementation.
    Parallel atomicAdd operation in CUDA is converted to optimized torch.einsum (matrix contraction).
    """
    delta_xyz = torch.einsum('bl,lnc->bnc', weight, xyz_b)
    delta_rot = torch.einsum('bl,lnc->bnc', weight, rot_b)
    delta_feat = torch.einsum('bl,lncd->bncd', weight, feat_b)
    
    xyz_out = xyz.unsqueeze(0) + delta_xyz
    rot_out = rot.unsqueeze(0) + delta_rot
    feat_out = feat.unsqueeze(0) + delta_feat
    
    return xyz_out, rot_out, feat_out

linear_blending = linear_blending_torch
...
class GaussianModel:
    def __init__(self, model_config: Struct):
        ...
        if self.model_config.use_mlp_proj:
            ...
            self.weight_module = nn.Linear(self.model_config.num_basis_in, self.model_config.num_basis_blend, device='cpu')
    ...
    def load_ply(self, path: str, train: bool = False):
        ...
        linear_module = torch.from_numpy(linear_module).cpu()  # CPU로 변경
        ...
        if 'face_id' in p_names:
            ...
            self.binding_face_id = torch.from_numpy(binding_face_id).cpu() # CPU로 변경
        ...
        if len(bary_names) > 0:
            ...
            self.binding_face_bary = torch.from_numpy(binding_face_bary).cpu() # CPU로 변경
            print("Binding face barycentric loaded")

        self._xyz = torch.from_numpy(xyz).cpu() # CPU로 변경
        self._opacity = torch.from_numpy(opacity).cpu() # CPU로 변경    
        self._scaling = torch.from_numpy(scaling).cpu() # CPU로 변경
        self._rotation = torch.from_numpy(rotation).cpu() # CPU로 변경
        self._feature_dc = torch.from_numpy(feature_dc).transpose(1, 2).contiguous().cpu() # CPU로 변경

        self._xyz_b = torch.from_numpy(xyz_b).transpose(0, 1).contiguous().cpu() # CPU로 변경
        self._rotation_b = torch.from_numpy(rotation_b).transpose(0, 1).contiguous().cpu() # CPU로 변경
        self._feature_b = torch.from_numpy(f_dc_b).transpose(0, 1).contiguous().cpu() # CPU로 변경
