...
class FLAMEDataset(Dataset):
    ...
    def load_track_params(self):
        ...
        self.jaw_poses = rotation_6d_to_matrix(jaw_poses.cpu())
        self.eye_poses = rotation_6d_to_matrix(eye_poses.reshape(-1, 2, 6).cpu())   
    ...
    def transform_flame_params(self, flame_model: FLAME):
        shapes = self.shapes.cpu()
        exps = self.exps.cpu()

        joints = flame_model.compute_joint_location(shapes, exps)
        neck_joints = joints[:, 1]
        global_transls = torch.matmul(self.global_rots.cpu(), neck_joints.unsqueeze(-1)).squeeze(-1) - neck_joints
        global_transls += self.global_transls.cpu()
    ...
    def precompute_bind_mesh(self, flame_model: FLAME):
        verts = flame_model( # blendshape & articulate transform
                shape_params=self.shapes.cpu(),
            expression_params=self.exps.cpu(),
            neck_pose_params=self.neck_poses.cpu(),
            jaw_pose_params=self.jaw_poses.cpu(),
            eye_pose_params=self.eye_poses.cpu(),
            eyelid_params=self.eyelid_params.cpu()
        )
        verts = torch.matmul(verts, self.global_rots.transpose(-1, -2).cpu()) # global transform
        verts += self.global_transls.unsqueeze(1).cpu()
